...
@section('form-route'){{route('client-invoice-details.update',['clientInvoice'=>$clientInvoice->id,'client_invoice_detail'=>$clientInvoiceDetail->id])}}@endsection
...
<nav class="page-breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{!! route('client-invoice-details.index',['clientInvoice'=>$clientInvoice->id]) !!}">{{__('common.costs')}}</a></li>
...
<div class="row">
    <div class="tab-content border border-top-0 p-3 w-100" id="myTabContent">
        <div class="tab-pane fade show active" id="users-overview" role="tabpanel" aria-labelledby="costs-overview-tab">
            @include('pages.client_invoice.details.form', ['clientInvoice'=>$clientInvoice,'client_invoice_detail'=>$clientInvoiceDetail,'source' => 'update'])
        </div>
    </div>
</div>
@endsection
